...
import { InfoPanel, Modal, Tag } from './UI.jsx';
...
/* ============================================================
   ...
   Commercial table also owns the editable name + delete UI.
   ============================================================ */
function ScoringMatrix({ rubricKey, rubricLabel, weightKey, showLabel = false, editable = false }) {
  const { data, dispatch, log } = useData();
  const factors = data.rubrics[rubricKey].factors;
  ...
  // Count campaign + hard commitment references for delete-guard
                    ...
                    <td>
                      {editable ? (
                        <input
                          ...
                          style={{ fontWeight: 500, fontSize: '14px', width: '100%', minWidth: '120px' }}
          ...
          >
            <p style={{ fontSize: '14px', color: 'var(--ink-2)', lineHeight: 1.6 }}>
              {blocked ? (
                ...
                <>
                  This will permanently remove <strong>{confirmDelete.name}</strong> and its
                  scores from both rubrics.
                </>
              )}
            </p>
          </Modal>
